...
**Attribute taxonomy (re-verified 2026-07-01 against repo `prompts/get_base_attrs.py` + `get_aux_tags.py`):** each attribute is formatted `[selection type], [attribute type], [attribute name]`, where **attribute type has exactly TWO values — `main object` or `background`** (prompt text verbatim; repo-wide grep finds no `global` type), and selection type ∈ `single | multi | binary`. Examples the repo actually emits: `single, main object, object size`, `single, main object, object color`, `single, background, background lighting`, `single, background, dominant background color`. **Correction:** an earlier version of this note claimed a *third* `global` group (resolution/noise/brightness/camera-angle) — that was **unsupported by the repo and is wrong**; lighting is a `background` attribute, not a separate global one. The two groups map onto the generator's two-layer input: main-object → per-instance caption, background → global caption.
